  ...
  return (
    <main className="min-h-screen bg-background">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="space-y-6 order-2 md:order-1">
                ...
                <p className="text-xl md:text-2xl text-gray-500 font-medium font-body">
      ...
      <section className="py-16 bg-yellow">
        ...
        <h2 className="max-w-3xl mx-auto text-3xl md:text-4xl font-display font-bold text-foreground mb-12">
        About this activity
            </h2>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {christmasTour.included.map((item, index) => {
                const getIcon = (item: string) => {
                  if (item.toLowerCase().includes('pickup') || item.toLowerCase().includes('drop-off') || item.toLowerCase().includes('hotel')) return <MapPin className="w-5 h-5" />;
                  if (item.toLowerCase().includes('guide')) return <Users className="w-5 h-5" />;
                  if (item.toLowerCase().includes('lunch') || item.toLowerCase().includes('food') || item.toLowerCase().includes('tasting') || item.toLowerCase().includes('pastries') || item.toLowerCase().includes('beverages')) return <UtensilsCrossed className="w-5 h-5" />;
                  if (item.toLowerCase().includes('workshop') || item.toLowerCase().includes('materials') || item.toLowerCase().includes('decoration')) return <Gift className="w-5 h-5" />;
                  if (item.toLowerCase().includes('insurance')) return <Shield className="w-5 h-5" />;
                  if (item.toLowerCase().includes('entry') || item.toLowerCase().includes('visit')) return <Camera className="w-5 h-5" />;
