import { privateGroupTour } from "@/data/tourDescription";
import TourPath from "@/components/TourPath";
import { CheckCircle2, MapPin, Users, Clock, Languages, Sparkles } from "lucide-react";

const PrivateGroupTour = () => {
  return (
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto space-y-6">
            <p className="text-lg text-muted-foreground leading-relaxed text-center font-body">
              {privateGroupTour.description}
            </p>
            <p className="text-foreground leading-relaxed text-center font-body">
              {privateGroupTour.intro}
            </p>
          </div>
        </div>
      </section>

      <TourPath stops={privateGroupTour.tourPath} />
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {privateGroupTour.included.map((item, index) => {
                const getIcon = (item: string) => {
                  if (item.toLowerCase().includes('guide')) return <Users className="w-5 h-5" />;
                  if (item.toLowerCase().includes('customizable') || item.toLowerCase().includes('itinerary')) return <Sparkles className="w-5 h-5" />;
                  if (item.toLowerCase().includes('time') || item.toLowerCase().includes('schedule')) return <Clock className="w-5 h-5" />;
                  if (item.toLowerCase().includes('hotel') || item.toLowerCase().includes('pickup') || item.toLowerCase().includes('drop-off')) return <MapPin className="w-5 h-5" />;
                  if (item.toLowerCase().includes('group') || item.toLowerCase().includes('people')) return <Users className="w-5 h-5" />;
                  if (item.toLowerCase().includes('language')) return <Languages className="w-5 h-5" />;
...
export default PrivateGroupTour;
